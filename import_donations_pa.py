...
try:
    ...
    for row in csvreader:
        ...
        if row_dict[contributor_name_field] == 'Total of Contributions not exceeding $100':
            ...
            #    mailing_address = '{}, {}'.format(mailing_address, row_dict[contributor_address2_field])

            mailing_address = '{}, {}, {} {}'.format(mailing_address, row_dict[contributor_city_field], row_dict[contributor_state_field], row_dict[zip_code_field]).strip(' ,')
            ...
            if mailing_address == '':
                ...
                try:
                    ...
                    # Do not use 'doctor' as street type for 'drive' (DR abbreviation)
                    # Do not use 'saint' as a street type for 'street' (ST abbr)
                    # Use the state that has two letter abbreviation

                    valid_address = False

                    for address in expanded_address:
                        parsed_address = parse_address(address)
                        address_dict = {d[1]: d[0] for d in parsed_address}
                        if 'house_number' in address_dict and 'road' in address_dict and 'state' in address_dict and 'city' in address_dict and  'postcode' in address_dict:
                            if address_dict['road'][-6:] != 'doctor' and address_dict['city'][:5] != 'saint' and len(address_dict['state']) == 2:
                                valid_address = True
                                break

                    if not valid_address:
                        #print('ERROR: No libpostal address match for {} at {}'.format(mailing_address, line_num))
    ...
    print('Exception line {}: {}'.format(line_num, e))
